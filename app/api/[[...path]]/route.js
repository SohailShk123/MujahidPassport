...
const guard = (request) =>
  isAuthed(request) ? null : json({ error: "Unauthorized" }, 401);
  ...
  try {
    ...
    if (path.endsWith("/api/stats")) {
      ...
      const pending = customers.reduce(
        (a, c) =>
          a +
          Math.max(0, Number(c.totalAmount || 0) - Number(c.paidAmount || 0)),
        0,
      );

      return json({
        totalCustomers: customers.length,
        newCases: customers.filter((c) => c.status === "New").length,
        documentsPending: customers.filter(
          (c) => c.status === "Documents Pending",
        ).length,
        appointments: customers.filter((c) => c.appointmentDate === today)
          .length,
    ...
    if (path.endsWith("/api/appointments")) {
      return json(customers.filter((c) => c.appointmentDate).map(safe));
    }
...
export async function POST(request) {
  ...
  if (path.endsWith("/api/login")) {
    ...
    response.cookies.set("passport_session", "demo-session", {
      httpOnly: true,
      sameSite: "lax",
      maxAge: 60 * 60 * 12,
      path: "/",
  ...
  try {
    ...
    if (path.endsWith("/api/customers")) {
      const count = await database.collection("customers").countDocuments();

      const year = new Date().getFullYear();

      const fileNumber = `PS-${year}-${String(count + 1).padStart(4, "0")}`;

      const totalAmount = Number(body.governmentFee || 0);
    ...
    if (path.endsWith("/api/payments")) {
      ...
      const customer = await database.collection("customers").findOne({
        _id: require("mongodb").ObjectId.createFromHexString(body.customerId),
      });
      ...
      const paidAmount = Number(customer.paidAmount || 0) + amount;

      const activity = [
        ...(customer.activity || []),
        {
          type: "Payment Received",
          text: `₹${amount.toLocaleString("en-IN")} payment received`,
          at: new Date().toISOString(),
        },
      ];
      ...
      await database.collection("customers").updateOne(
        ...
        },
        {
          $set: {
            paidAmount,
            balance: Math.max(0, Number(customer.totalAmount) - paidAmount),
    ...
    if (path.endsWith("/api/documents")) {
      ...
      return json(
        {
          error: "Use the multipart upload endpoint.",
        },
        400,
      );
    }
